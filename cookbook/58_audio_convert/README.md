...
cp cookbook/51_audio_summary/sample/clip.mp3 \
   cookbook/58_audio_convert/sample/clip.mp3

# Option B — generate one with espeak (Linux)
espeak "Audio format conversion test for SPL 3.0." \
    -w cookbook/58_audio_convert/sample/clip.wav

# Option C — generate one with OpenAI TTS
python -c "
from openai import OpenAI; import pathlib
out = pathlib.Path('cookbook/58_audio_convert/sample/clip.mp3')
...
python cookbook/58_audio_convert/run.py \
    --audio cookbook/58_audio_convert/sample/clip.mp3 \
    --target-format wav
...
python cookbook/58_audio_convert/run.py \
    --audio cookbook/58_audio_convert/sample/clip.wav \
...
spl run cookbook/58_audio_convert/audio_convert.spl \
    --tools cookbook/tools/registry.py

# MP3 → OGG
spl run cookbook/58_audio_convert/audio_convert.spl \
    --tools cookbook/tools/registry.py \
    -p audio=cookbook/58_audio_convert/sample/clip.mp3 \
    -p target_format=ogg \
    -p bitrate=128k
...
|-----------|------|---------|-------------|
| `@audio` | AUDIO | `cookbook/58_audio_convert/sample/clip.wav` | Source audio file path |
...
| `@output_dir` | TEXT | `cookbook/58_audio_convert/outputs` | Directory to write converted file |
| `@log_dir` | TEXT | `cookbook/58_audio_convert/logs-spl` | Directory for log output |
