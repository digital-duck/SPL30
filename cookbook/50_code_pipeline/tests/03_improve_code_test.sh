...
run_test() {
    ...
    save_run_script "$run_script" "$PIPELINE_DIR/03_improve_code.spl" ollama \
        code "$code" feedback "$feedback" lang "python" model "$MODEL" log_dir "$LOG_DIR/$name"
    local -a cmd=(spl3 run "$PIPELINE_DIR/03_improve_code.spl"
        --adapter ollama
        --param "code=$code"
