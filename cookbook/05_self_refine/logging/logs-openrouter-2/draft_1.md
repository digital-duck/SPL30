...
- The **Encoder** reads the entire input sequence and produces a rich set of contextual representations.
- The **Decoder** generates the output sequence one token at a time, attending to both its own previous outputs and the encoder's representations.

Modern variants specialize this structure: **BERT** uses only the encoder (optimized for understanding tasks like classification), while **GPT** uses only the decoder (optimized for generation). Understanding the full encoder-decoder model reveals the design principles behind all of them.
...
Running a single attention operation captures only one type of relationship at a time. **Multi-Head Attention** runs several attention operations in parallel, each learning different relationship patterns simultaneously.
...
Each "head" has its own learned projection matrices (W_Q, W_K, W_V), so they independently learn to attend to different aspects of the input:
...
In the original paper, 8 heads were used with `d_k = d_model / h = 64` dimensions each. After parallel computation, all heads are concatenated and projected back to `d_model` dimensions via W_O.

This is analogous to how CNNs use multiple filters to detect different visual features simultaneously — the model learns which "types" of attention are useful, rather than having them specified by hand.

---

## Step 5: The Feed-Forward Network (FFN)

After attention, each token's representation passes through a **position-wise Feed-Forward